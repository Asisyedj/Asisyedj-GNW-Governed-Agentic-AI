...
/**
 * Durable side of the policy gateway. Nonce consumption and interlock state are
 * persisted so replay protection and the kill switch survive restarts and hold
 * across every instance of a horizontally scaled deployment.
 */
...
export class GovernanceService {
  ...
  digest(request: GovernanceRequest) {
...
export class MemoryGovernanceStores implements GovernanceStores {
  ...
  async reserveBudget(_taskId: number, _grantNonce: string, _tokens: number, _bytes: number) { return true; }
...
export function newGrant(input: Omit<GovernanceRequest, "requestId" | "nonce" | "issuedAt" | "expiresAt"> & { ttlMs?: number }): GovernanceRequest {
  ...
  return {
    ...
    nonce: randomUUID(),
    issuedAt,
    expiresAt: issuedAt + (input.ttlMs ?? 600_000),
  };
}
