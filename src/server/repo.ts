...
export async function createDeepResearchRun(db: Db, input: {
  taskId: number; requestId: string; originalPrompt: string; clarificationsJson: string;
  researchInstructions: string; toolPolicyJson: string; status?: string;
}) {
  const stamp = now();
  return db.insert(
    `INSERT INTO deep_research_runs (task_id, request_id, status, original_prompt, clarifications_json, research_instructions, tool_policy_json, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [input.taskId, input.requestId, input.status ?? "queued", input.originalPrompt, input.clarificationsJson, input.researchInstructions, input.toolPolicyJson, stamp, stamp],
  );
}

export async function updateDeepResearchRun(db: Db, runId: number, values: Partial<{
  responseId: string | null; status: string; resultText: string | null; responseJson: string | null; errorCode: string | null; completedAt: number | null;
  researchInstructions: string; toolPolicyJson: string; clarificationsJson: string;
}>) {
  const sets: string[] = ["updated_at = ?"];
  const params: unknown[] = [now()];
  if (values.responseId !== undefined) { sets.push("response_id = ?"); params.push(values.responseId); }
  if (values.status !== undefined) { sets.push("status = ?"); params.push(values.status); }
  if (values.resultText !== undefined) { sets.push("result_text = ?"); params.push(values.resultText); }
  if (values.responseJson !== undefined) { sets.push("response_json = ?"); params.push(values.responseJson); }
  if (values.errorCode !== undefined) { sets.push("error_code = ?"); params.push(values.errorCode); }
  ...
  params.push(runId);
  await db.run(`UPDATE deep_research_runs SET ${sets.join(", ")} WHERE id = ?`, params);
}

export async function getDeepResearchRun(db: Db, runId: number, taskId: number) {
  return db.get<DeepResearchRunRow>("SELECT * FROM deep_research_runs WHERE id = ? AND task_id = ?", [runId, taskId]);
}

export async function listDeepResearchRuns(db: Db, taskId: number) {
  return db.all<DeepResearchRunRow>("SELECT * FROM deep_research_runs WHERE task_id = ? ORDER BY id ASC", [taskId]);
}
