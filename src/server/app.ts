...
import { answerWithCitations, classifySensitiveAction, draftProjectPlan, OFFICE_PHASE1_SCOPE, verifySource } from "./office.js";
import { createResearchPlan, startDeepResearch, refreshDeepResearch, getDeepResearch, cancelDeepResearch, modelAssistRewrite } from "./deep-research.js";
...
export async function createApp(env: Env = ENV, dbPromise: Promise<Db> = getDb(env)) {
  ...
  app.get("/api/tasks/:taskId", auth(), asyncRoute(async (req, res) => {
    ...
    const [messages, runs, approvals, videoJobs, artifacts, audit, deepResearchRuns] = await Promise.all([
      ...
      repo.listDeepResearchRuns(req.db, taskId),
    ]);
    return res.json({ task, messages, runs, approvals: approvals.filter(item => item.task_id === taskId), videoJobs, artifacts, audit, deepResearchRuns });
  }));

  /* ---------------------------------------------------------- approvals */

  app.post("/api/tasks/:taskId/deep-research/plan", auth(), asyncRoute(async (req, res) => {
    const taskId = Number(req.params.taskId);
    if (!Number.isInteger(taskId) || taskId <= 0) return res.status(400).json({ error: "invalid_task_id" });
    const task = await repo.getTaskForUser(req.db, taskId, req.user!.workspaceId);
    if (!task) return res.status(404).json({ error: "task_not_found" });
    const parsed = z.object({ useModelRewrite: z.boolean().default(true) }).safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: "invalid_input", detail: parsed.error.flatten() });
    const created = await createResearchPlan(req.db, req.user!, task.id, task.prompt, task.purpose, task.classification, env);
    if (parsed.data.useModelRewrite) {
      const clarificationPayload = created.plan.clarificationQuestions;
      const instructions = await modelAssistRewrite(req.db, req.user!, task.id, created.plan, {}, task.purpose, task.classification, env);
      await req.db.run("UPDATE deep_research_runs SET research_instructions = ?, updated_at = ? WHERE id = ?", [instructions, Date.now(), created.runId]);
      return res.status(201).json({ ...created, clarificationQuestions: clarificationPayload, researchInstructions: instructions, modelRewrite: Boolean(env.llmApiKey) });
    }
    return res.status(201).json({ ...created, researchInstructions: undefined, modelRewrite: false });
  }));

  app.post("/api/tasks/:taskId/deep-research/start", auth(), asyncRoute(async (req, res) => {
    const taskId = Number(req.params.taskId);
    if (!Number.isInteger(taskId) || taskId <= 0) return res.status(400).json({ error: "invalid_task_id" });
    const task = await repo.getTaskForUser(req.db, taskId, req.user!.workspaceId);
    if (!task) return res.status(404).json({ error: "task_not_found" });
    const parsed = z.object({
      ...
      vectorStoreIds: z.array(z.string().min(1).max(200)).max(2).optional(),
      useCodeInterpreter: z.boolean().optional(),
      useMcp: z.boolean().default(false),
      maxToolCalls: z.number().int().min(1).max(100).optional(),
      clarificationAnswers: z.record(z.string().max(4000)).optional(),
    }).safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: "invalid_input", detail: parsed.error.flatten() });
    const result = await startDeepResearch(req.db, req.user!, task.id, parsed.data, task.purpose, task.classification, env);
    return res.status(result.status === "PENDING_RECONCILIATION" ? 409 : 202).json(result);
  }));

  app.post("/api/tasks/:taskId/deep-research/:runId/cancel", auth(), asyncRoute(async (req, res) => {
    const taskId = Number(req.params.taskId);
    const runId = Number(req.params.runId);
    if (!Number.isInteger(taskId) || !Number.isInteger(runId) || taskId <= 0 || runId <= 0) return res.status(400).json({ error: "invalid_research_id" });
    const task = await repo.getTaskForUser(req.db, taskId, req.user!.workspaceId);
    if (!task) return res.status(404).json({ error: "task_not_found" });
    const result = await cancelDeepResearch(req.db, req.user!, task.id, runId, task.purpose, task.classification, env);
    return res.status(200).json(result);
  }));

  app.get("/api/tasks/:taskId/deep-research/:runId", auth(), asyncRoute(async (req, res) => {
    const taskId = Number(req.params.taskId);
    const runId = Number(req.params.runId);
    if (!Number.isInteger(taskId) || !Number.isInteger(runId) || taskId <= 0 || runId <= 0) return res.status(400).json({ error: "invalid_research_id" });
    const task = await repo.getTaskForUser(req.db, taskId, req.user!.workspaceId);
    if (!task) return res.status(404).json({ error: "task_not_found" });
    const refreshed = await refreshDeepResearch(req.db, req.user!, task.id, runId, task.purpose, task.classification, env);
    const detail = await getDeepResearch(req.db, task.id, runId);
    return res.json({ current: refreshed, run: detail });
  }));
  ...
  app.get("/api/office/scope", auth(), (_req, res) => {
    res.json({ phase: "PHASE_1", scope: OFFICE_PHASE1_SCOPE, sideEffects: "DENIED_BY_DEFAULT" });
  });

  app.post("/api/office/documents", auth(), asyncRoute(async (req, res) => {
    const parsed = z.object({
    ...
    if (!parsed.success) return res.status(400).json({ error: "invalid_input", detail: parsed.error.flatten() });
    if (parsed.data.classification === "restricted") {
      await appendAudit(req.db, { actorUserId: req.user!.id, eventType: "office_document_ingest", decision: "STOP", reason: "restricted_document_requires_owner_review", payload: { title: parsed.data.title } });
      return res.status(423).json({ error: "restricted_document_requires_owner_review" });
    }
    const now = Date.now();
    const id = await req.db.insert(
      "INSERT INTO office_documents (workspace_id, title, content, source_ref, classification, created_by, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      [req.user!.workspaceId, parsed.data.title, parsed.data.content, parsed.data.sourceRef, parsed.data.classification, req.user!.id, now, now],
    );
    await appendAudit(req.db, { actorUserId: req.user!.id, eventType: "office_document_ingest", decision: "ALLOW", reason: "tenant_scoped_document_created", payload: { documentId: id, title: parsed.data.title, classification: parsed.data.classification } });
    return res.status(201).json({ id, title: parsed.data.title, sourceRef: parsed.data.sourceRef, classification: parsed.data.classification });
  }));

  app.get("/api/office/documents/search", auth(), asyncRoute(async (req, res) => {
    const query = typeof req.query.q === "string" ? req.query.q.trim() : "";
    if (query.length < 2 || query.length > 300) return res.status(400).json({ error: "invalid_query" });
    const like = `%${query.replace(/[\\%_]/g, "\\$&").replace(/'/g, "''")}%`;
    const rows = await req.db.all<{ id: number; title: string; content: string; source_ref: string; classification: string; updated_at: number }>(
      "SELECT id, title, content, source_ref, classification, updated_at FROM office_documents WHERE workspace_id = ? AND (title LIKE ? ESCAPE '\\' OR content LIKE ? ESCAPE '\\') ORDER BY updated_at DESC LIMIT 20",
      [req.user!.workspaceId, like, like],
    );
    await appendAudit(req.db, { actorUserId: req.user!.id, eventType: "office_document_search", decision: "ALLOW", reason: "tenant_scoped_read_only_search", payload: { queryLength: query.length, resultCount: rows.length } });
    return res.json({ query, results: rows.map(row => ({ ...row, content: row.content.slice(0, 4000) })) });
  }));

  app.post("/api/office/research/verify", auth(), asyncRoute(async (req, res) => {
    const parsed = z.object({
      answer: z.string().min(1).max(20_000),
      sources: z.array(z.object({
        id: z.string().min(1).max(120), url: z.string().min(1).max(2000), title: z.string().min(1).max(500), content: z.string().max(50_000),
        retrievedAt: z.string().optional(), publishedAt: z.string().optional(), sourceType: z.enum(["INTERNAL", "GOVERNMENT", "STANDARD", "RESEARCH", "OFFICIAL_DOCS", "COMPANY", "SECONDARY"]).optional(),
      })).max(20),
    }).safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: "invalid_input", detail: parsed.error.flatten() });
    const verified = parsed.data.sources.map(verifySource);
    const result = answerWithCitations(parsed.data.answer, verified);
    await appendAudit(req.db, { actorUserId: req.user!.id, eventType: "office_research_verified", decision: result.abstained ? "STOP" : "ALLOW", reason: result.abstained ? "insufficient_verified_evidence" : "citations_bound_to_source_hashes", payload: { sourceCount: verified.length, citationCount: result.citations.length, sourceIds: verified.map(source => source.id) } });
    return res.status(result.abstained ? 422 : 200).json({ ...result, sources: verified.map(source => ({ id: source.id, title: source.title, urlHost: source.urlHost, support: source.support, limitations: source.limitations, contentHash: source.contentHash })) });
  }));

  app.post("/api/office/plan/draft", auth(), asyncRoute(async (req, res) => {
    const parsed = z.object({ prompt: z.string().min(8).max(12_000), deliverables: z.array(z.string().max(500)).max(20).optional(), constraints: z.array(z.string().max(500)).max(20).optional() }).safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: "invalid_input", detail: parsed.error.flatten() });
    const plan = draftProjectPlan(parsed.data);
    await appendAudit(req.db, { actorUserId: req.user!.id, eventType: "office_plan_draft", decision: "ALLOW", reason: "draft_only_human_review_required", payload: { objective: plan.objective, taskCount: plan.tasks.length } });
    return res.json(plan);
  }));

  app.post("/api/office/action-check", auth(), asyncRoute(async (req, res) => {
    const parsed = z.object({ action: z.string().min(1).max(2000) }).safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: "invalid_input" });
    const decision = classifySensitiveAction(parsed.data.action);
    await appendAudit(req.db, { actorUserId: req.user!.id, eventType: "office_sensitive_action_check", decision: decision.allowed ? "ALLOW" : "STOP", reason: decision.reason ?? "read_or_draft_action", payload: { actionLength: parsed.data.action.length } });
    return res.status(decision.allowed ? 200 : 423).json({ ...decision, humanApprovalRequired: !decision.allowed });
  }));
