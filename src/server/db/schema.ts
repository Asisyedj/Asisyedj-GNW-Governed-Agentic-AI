...
export function schemaStatements(dialect: Dialect): string[] {
  ...
  const statements = [
    ...
    `CREATE INDEX IF NOT EXISTS ix_deep_research_task ON deep_research_runs (task_id, id)`,
    ...
    `CREATE INDEX IF NOT EXISTS ix_office_documents_workspace ON office_documents (workspace_id, updated_at)`,
  ...
  if (dialect === "postgres") {
    statements.push(
      ...
      `DROP POLICY IF EXISTS gnw_office_documents_tenant ON office_documents`,
      `CREATE POLICY gnw_office_documents_tenant ON office_documents USING (EXISTS (SELECT 1 FROM workspaces w WHERE w.id = office_documents.workspace_id AND w.tenant_key = gnw_current_tenant())) WITH CHECK (EXISTS (SELECT 1 FROM workspaces w WHERE w.id = office_documents.workspace_id AND w.tenant_key = gnw_current_tenant()))`,
    );
  }
  return statements;
}
